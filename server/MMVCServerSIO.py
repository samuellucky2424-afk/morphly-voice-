...
def applyRealtimeCpuTuning():
    ...
    if env_threads:
        ...
        cpu_threads = cpu_count if cpu_count <= 2 else max(1, min(4, cpu_count - 1))

    try:
        torch.set_num_threads(cpu_threads)
        torch.set_num_interop_threads(max(1, min(2, cpu_threads)))
    except Exception as e:
        logger.warning(f"[Voice Changer] CPU thread tuning skipped: {e}")


applyRealtimeCpuTuning()


def setupArgParser():
    parser = argparse.ArgumentParser()
    parser.add_argument("--logLevel", type=str, default="error", help="Log level info|critical|error. (default: error)")
    parser.add_argument("-p", "--port", dest="p", type=int, default=18888, help="port")
